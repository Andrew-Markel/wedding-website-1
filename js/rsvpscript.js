...
async function getSheetsApi(inviteCodeInput) {
    ...
    if (jsonData.formal_name.toString() !== "") {
        ...
        `${jsonData.formal_name.toString()}<br>` +
        `Welcome to Our Wedding<br>` + 
        `It is our honor to invite you, along with the esteemed guest(s) listed below, to share in this special day with us.<br>` +
        `Kindly indicate the guest(s) for whom you are responding. If desired, enter their contact information to receive event updates.<br>`;
        document.getElementById('information_message').style.display = 'inline-flex';
...
async function getData(inviteCodeInput) {
    try {
      const response = await fetch(`https://script.google.com/macros/s/AKfycbzDJOYvfamZNkpF1aqGIA2AS9UFYBmdZ8GgpEpUxoYeRhWYp6DfTLElBHCWuscjOcTn9A/exec?invite_code=${inviteCodeInput.toString()}`);
      if (!response.ok) {
...
async function toggleRequiredField() {
    var radioYes = document.getElementById('rsvp_yes');
...
function showForm(checkbox) {
    ...
    if (document.getElementById(checkbox.id).checked) {
        ...
        if (document.querySelector(`label[for="invited_${id}"]`).innerText == "+1") {
...
async function postSheetsApi(inviteCodeInput, data) {
    ...
    try {
        const response = await fetch(`https://script.google.com/macros/s/AKfycbzDJOYvfamZNkpF1aqGIA2AS9UFYBmdZ8GgpEpUxoYeRhWYp6DfTLElBHCWuscjOcTn9A/exec?${queryString}`, {
            method: 'POST',
        });
